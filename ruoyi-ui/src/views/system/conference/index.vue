  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="conferenceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="状态" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_conference_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="会议图片" prop="image">
...
<script>
import { listConference, getConference, delConference, addConference, updateConference } from "@/api/system/conference";

export default {
  name: "Conference",
  dicts: ['sys_conference_status'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getConference(id).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除会议管理编号为"' + ids + '"的数据项？').then(function() {
        return delConference(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/conference/export', {
        ...this.queryParams
